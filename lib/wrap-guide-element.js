...
module.exports = class WrapGuideElement {
  ...
  constructor(editor, editorElement, when) {
    ...
    this.element = editorElement.ownerDocument.createElement("div");
  ...
  handleEvents() {
    ...
    this.subscriptions.add(
      lumine.config.onDidChange("editor.fontSize", async () => {
    ...
    this.subscriptions.add(
      lumine.workspace.onDidChangeActiveTextEditor((activeEditor) => {
        if (activeEditor === this.editor) {
          this.scheduleUpdateGuide(updateGuideCallback);
        }
      }),
    );
    this.subscriptions.add(
  ...
  scheduleUpdateGuide(callback = () => this.updateGuide()) {
    if (this.updateFrame != null) this.updateFrameWindow?.cancelAnimationFrame(this.updateFrame);
    this.updateFrameWindow = this.editorElement.ownerDocument.defaultView;
    this.updateFrame = this.updateFrameWindow.requestAnimationFrame(() => {
      this.updateFrame = null;
      this.updateFrameWindow = null;
      callback();
    });
  }
  ...
  beginWindowSurfaceTransition() {
    ...
    this.updateFrameWindow = null;
    const finish = async () => {
      if (this.element.ownerDocument !== this.editorElement.ownerDocument) {
        this.editorElement.ownerDocument.adoptNode(this.element);
      }
      this.attachToLines();
      await this.updateGuide();
    };
  ...
  handleConfigEvents() {
    ...
    const updatePreferredLineLengthCallback = async (args) => {
      ...
      let columns = lumine.config.get("wrap-guide.columns", {
        scope: this.editor.getRootScopeDescriptor(),
      });
      if (columns.length > 0) {
    ...
    const updateGuidesCallback = async (args) => {
      ...
      if (columns != null ? columns.length : undefined) {
        lumine.config.set("wrap-guide.columns", columns);
        if (lumine.config.get("wrap-guide.modifyPreferredLineLength")) {
          lumine.config.set("editor.preferredLineLength", columns[columns.length - 1], {
            scopeSelector: `.${this.editor.getGrammar().scopeName}`,
          });
        }
        return await this.updateGuide();
  ...
  getGuidesColumns(_path, _scopeName) {
    let left;
    const columns =
      (left = lumine.config.get("wrap-guide.columns", {
        scope: this.editor.getRootScopeDescriptor(),
      })) != null
        ? left
        : [];
  ...
  isEnabled() {
    let left;
    return (left = lumine.config.get("wrap-guide.enabled", {
      scope: this.editor.getRootScopeDescriptor(),
    })) != null
      ? left
      : true;
  ...
  appendGuide(column) {
    ...
    const guide = this.element.ownerDocument.createElement("div");
